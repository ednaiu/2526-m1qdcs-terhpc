...
void sscal_scalar(int n, float alpha, float *x) { for (int i = 0; i < n; i++) x[i] *= alpha; }
...
void sscal(int n, float alpha, float *x) { sscal_avx2(n, alpha, x); }

void scopy_scalar(int n, const float *x, float *y) { for (int i = 0; i < n; i++) y[i] = x[i]; }
...
void scopy(int n, const float *x, float *y) { scopy_avx2(n, x, y); }

void sswap_scalar(int n, float *x, float *y) { for (int i = 0; i < n; i++) { float t = x[i]; x[i] = y[i]; y[i] = t; } }
...
void sswap(int n, float *x, float *y) { sswap_avx2(n, x, y); }

void saxpy_scalar(int n, float alpha, const float *x, float *y) { for (int i = 0; i < n; i++) y[i] += alpha * x[i]; }
...
/* sdot */
float sdot_scalar(int n, const float *x, const float *y) { float s = 0.0f; for (int i = 0; i < n; i++) s += x[i]*y[i]; return s; }
...
float sdot(int n, const float *x, const float *y) { return sdot_avx2(n, x, y); }

/* snrm2 */
float snrm2_scalar(int n, const float *x) { float s = 0.0f; for (int i = 0; i < n; i++) s += x[i]*x[i]; return sqrtf(s); }
...
float snrm2(int n, const float *x) { return snrm2_avx2(n, x); }

/* sasum */
float sasum_scalar(int n, const float *x) { float s = 0.0f; for (int i = 0; i < n; i++) s += fabsf(x[i]); return s; }
...
float sasum(int n, const float *x) { return sasum_avx2(n, x); }
...
int isamax_avx2(int n, const float *x) { return isamax_scalar(n, x); } /* stub for avx2 */
int isamax(int n, const float *x) { return isamax_scalar(n, x); }

/* srot */
void srot_scalar(int n, float *x, float *y, float c, float s) {
    for (int i = 0; i < n; i++) { float xi = x[i], yi = y[i]; x[i] = c*xi + s*yi; y[i] = -s*xi + c*yi; }
}
void srot_avx2(int n, float *x, float *y, float c, float s) { srot_scalar(n, x, y, c, s); } /* stub */
void srot(int n, float *x, float *y, float c, float s) { srot_scalar(n, x, y, c, s); }
