...
\begin{frame}{Where We Left Off --- Week 2 Baseline}
  \begin{columns}
    ...
    \column{0.47\textwidth}
    \begin{block}{Week 2 vs OpenBLAS (20T, 6x16 TASK1)}
      \footnotesize
      \begin{tabular}{lrrc}
        \toprule
        Size & Ours & OB & Ratio \\
        \midrule
        $64^3$   & 34.1 & 49.6 & $0.69\times$ \\
        $128^3$  & 78.9 & 79.4 & $0.99\times$ \\
        $256^3$  & 274  & 151.7 & \textcolor{good}{$1.81\times$} \\
        $512^3$  & 568  & 381.2 & \textcolor{good}{$1.49\times$} \\
        $1024^3$ & 796  & 493.6 & \textcolor{good}{$1.61\times$} \\
        $2048^3$ & 884  & 535.8 & \textcolor{good}{$1.65\times$} \\
        \bottomrule
      \end{tabular}
    \end{block}
    \vspace{0.1cm}
    \scriptsize
    \textit{Both measured in same benchmark,\\OPENBLAS\_NUM\_THREADS = OMP\_NUM\_THREADS = 20,\\median of 7 runs.}
  \end{columns}
\end{frame}
...
%% =========================================================
\begin{frame}[fragile]{Solution P1: Loop Unrolling x4}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}[fragile]{Solution P2: Software Prefetching (Next Tile)}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}[fragile]{Solution P3: Non-Temporal Stores for C}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}[fragile]{Solution P4: ASM Kernel --- GCC 30-Operand Limit}
  \begin{columns}
...
%% =========================================================
\begin{frame}[fragile]{Solution P5: Small-Matrix Bypass}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}[fragile]{Solution P6: TASK2 Parallel Reduction}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}[fragile]{Solution P7: Adaptive MC for Thread Balance}
  \begin{columns}
    \column{0.50\textwidth}
...
\begin{frame}{Progressive Benchmark Results}
  \textbf{Conditions:} 20 threads, 6x16 TASK1, median of 7 runs.
  Same binary used for both our code and OpenBLAS (\texttt{OPENBLAS\_NUM\_THREADS=20}).
  \vspace{0.15cm}
  ...
  \begin{table}
    ...
    \begin{tabular}{lrrrrrc}
      ...
      \cmidrule(lr){2-5}
      \textbf{Size} & \textbf{Baseline} & \textbf{+P5} & \textbf{+P6} & \textbf{+P7 (final)} & \textbf{OpenBLAS} & \textbf{Final/OB} \\
      \midrule
  ...
  \vspace{0.1cm}
  \footnotesize
  \textbf{P5} = small-matrix bypass \quad \textbf{P6} = parallel TASK2 reduction \quad \textbf{P7} = adaptive MC\\
  Columns are \textit{cumulative} (each column includes all previous fixes).\\
  \textit{Note: non-monotone cells (e.g.\ $1024^3$ P5 = 580 $<$ baseline 796) are explained by MC-tuning
  interactions resolved by P7.}
\end{frame}
...
\begin{frame}{Final Results vs OpenBLAS}
  \begin{columns}
    ...
    $128^3$ and above: \textcolor{good}{\textbf{we win consistently}}\\[0.15cm]
    \textit{Methodology: same C binary calls\\
    our \texttt{sgemm\_ex} and \texttt{cblas\_sgemm}\\
    back-to-back. Both use 20 threads.\\
    Median of 7 timed runs each.}
  \end{columns}
\end{frame}
