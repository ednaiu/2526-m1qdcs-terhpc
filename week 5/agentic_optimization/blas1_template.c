/*
 * blas1.c — Week 4: Optimised BLAS 1 kernels (TEMPLATE FOR AGENTIC LOOP)
 */
#define _GNU_SOURCE
#include <stdlib.h>
#include <string.h>
...
void sswap(int n, float *x, int incx, float *y, int incy) {
    ...
    if (incx == 1 && incy == 1) {
        ...
        for (; i <= n - 8; i += 8) {
            ...
            _mm256_storeu_ps(x + i, vy); _mm256_storeu_ps(y + i, vx);
        }
...
float sdot(int n, const float *x, int incx, const float *y, int incy) {
    ...
    if (incx == 1 && incy == 1) {
        ...
        float s = 0.0f; for (int i = 0; i < n; i++) s += x[i * incx] * y[i * incy]; return s;
    }
}
...
float snrm2(int n, const float *x, int incx) {
    ...
    if (incx == 1) {
        ...
        float s = 0.0f; for (int i = 0; i < n; i++) { float v = x[i*incx]; s += v*v; } return sqrtf(s);
    }
}
...
float sasum(int n, const float *x, int incx) {
    ...
    if (incx == 1) {
        ...
        float s = 0.0f; for (int i = 0; i < n; i++) s += fabsf(x[i*incx]); return s;
    }
}
...
int isamax(int n, const float *x, int incx) {
    ...
    for (int i = 1; i < n; i++) { float v = fabsf(x[i*incx]); if (v > maxv) { maxv = v; idx = i; } }
    return idx;
}
...
void srot(int n, float *x, int incx, float *y, int incy, float c, float s) {
    for (int i = 0; i < n; i++) {
        float xi = x[i*incx], yi = y[i*incy]; x[i*incx] = c*xi + s*yi; y[i*incy] = -s*xi + c*yi;
    }
}
